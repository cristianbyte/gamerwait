...
export default function TrackedGameCard({ title, relase_date, followers }) {
  ...
  return (
    <div className=" cursor-pointer bg-transparent w-full rounded-lg p-3 border border-white/5 hover:border-neon/40 transition-all-duration-100">
      <div className="w-full flex justify-between items-center mb-3">
        <p className="text-xl font-sans text-text leading-tight">{title}</p>
        <svg
      ...
      <div className="flex justify-between flex-row-reverse items-center">
        ...
        <div className="flex justify-between mt-1 text-xs text-muted">
          <span className="text-xs font-sans">{relase_date}</span>
        </div>
      </div>
    </div>
  );
